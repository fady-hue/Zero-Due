...
const mockCases = [
  ...
  { id: "ZD-009", debtor: "Turki Al-Rashidi", phone: "+966 56 901 2345", amount: 67500, client: "Raqamyah", status: "PROMISE_TO_PAY", daysOld: 44, agent: "Omar T.", score: 61 },
  { id: "ZD-010", debtor: "Lina Al-Harbi", phone: "+966 50 012 3456", amount: 14300, client: "Tamara", status: "CONTACTED", daysOld: 18, agent: "Ahmed M.", score: 74 },
];
...
export default function CasesPage() {
  ...
  const filtered = mockCases.filter(c => {
    const matchSearch = !search || c.debtor.toLowerCase().includes(search.toLowerCase()) || c.id.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#8888A0] text-sm mt-0.5">{filtered.length} of {mockCases.length} cases</p>
        </div>
            ...
            <button
              ...
              className={`text-xs px-3 py-2 rounded-lg font-medium transition-all ${
                activeStatus === s
                  ...
                  : "bg-[#111118] border border-[#1E1E2E] text-[#8888A0] hover:text-white"
              }`}
            >
              {s.replace(/_/g, " ")}
            </button>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#1E1E2E]">
                {["Case ID", "Debtor", "Amount", "Client", "Status", "Days OD", "Agent", "Score", ""].map(h => (
            ...
            <tbody className="divide-y divide-[#1E1E2E]">
              {filtered.map((c, i) => (
                <motion.tr
                  ...
                  className="hover:bg-[#16161F] transition-colors group"
                >
                  <td className="px-4 py-3.5 text-[#6C5CE7] font-mono text-sm whitespace-nowrap">{c.id}</td>
                  <td className="px-4 py-3.5">
                    <div className="text-white text-sm font-medium whitespace-nowrap">{c.debtor}</div>
                    <div className="text-[#4A4A5A] text-xs">{c.phone}</div>
                  </td>
                  <td className="px-4 py-3.5 text-white text-sm font-semibold whitespace-nowrap">SAR {c.amount.toLocaleString()}</td>
                  <td className="px-4 py-3.5 text-[#8888A0] text-sm whitespace-nowrap">{c.client}</td>
                  <td className="px-4 py-3.5">
                    <span className={`text-xs px-2 py-1 rounded-lg font-medium whitespace-nowrap ${statusColors[c.status]}`}>
                      {c.status.replace(/_/g, " ")}
                    </span>
                  </td>
                  <td className="px-4 py-3.5 text-[#8888A0] text-sm">{c.daysOld}d</td>
                  <td className="px-4 py-3.5 text-[#8888A0] text-sm whitespace-nowrap">{c.agent}</td>
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 w-12 bg-[#1E1E2E] rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full"
                          style={{
                            width: `${c.score}%`,
                            backgroundColor: c.score > 70 ? "#00B894" : c.score > 50 ? "#FDCB6E" : "#E17055",
                          }}
                        />
                      </div>
                      <span
                  ...
                  <td className="px-4 py-3.5">
                    <Link
                      ...
                      className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 text-xs text-[#8888A0] hover:text-white p-1.5 rounded-lg hover:bg-[#1E1E2E]"
                    >
                      <Eye size={13} /> View
                    </Link>
                  </td>
